...
class CybrixToolsApp(ctk.CTk):
    ...
    def run_encryptdecrypt(self):
        ...
        ctk.CTkButton(btn_frame, text="Text file loaded", command=self.load_file).grid(row=0, column=0, padx=5)
        ctk.CTkButton(btn_frame, text="Encrypt", command=self.encrypt_text).grid(row=0, column=1, padx=5)
        ctk.CTkButton(btn_frame, text="Decrypt", command=self.decrypt_text).grid(row=0, column=2, padx=5)
        ctk.CTkButton(btn_frame, text="Save encrypted file", command=self.save_encrypted_file).grid(row=0, column=3, padx=5)
